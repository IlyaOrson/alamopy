...
def complete_opts(opts):
    ...
    if (
        ...
        and opts["noutputs"] is not None
        and opts["simulator"] is not None
    ):
...
def validate_opts(opts):
    ...
    if not case_1 and not case_2:
        raise RuntimeError(
            "Inadequate input options. ALAMO needs either \
    ...
    if len(opts["alm_file_name"]) > 1000:
        raise RuntimeError("File name should not exceed 1000 " "characters in length.")
